...
def search_airport(query: str) -> list:
    ...
    url = f"https://{api_host}/api/v1/flights/searchAirport?query={encoded_query}&locale=en-US"

    with httpx.Client() as client:
        res = client.get(url, headers=headers)
        if res.status_code != 200:
            print(f"Error: API responded with status code {res.status_code}")
...
def search_flights_real_api(
    ...
    url = (
        f"https://{api_host}/api/v2/flights/searchFlights?"
...
def search_flights(args: dict) -> dict:
    ...
    Falls back to mock data if real API returns an error.
    """
    load_dotenv(override=True)
    ...
    # If API key is available, try the real API first
    if api_key and api_key != "YOUR_DEFAULT_KEY":
        try:
            result = search_flights_real_api(args)
            # If no error in result, return it
            if "error" not in result:
                return result
            # Otherwise fall through to mock data
            print(f"Real API returned error, falling back to mock data: {result.get('error')}")
        except Exception as e:
            print(f"Real API failed, falling back to mock data: {e}")

    # Generate smart mock data as fallback
